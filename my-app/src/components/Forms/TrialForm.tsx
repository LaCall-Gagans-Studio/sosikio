...
export function TrialForm({ products }: Props) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!state.company || !state.name || !state.email || !anyProduct) {
      trackEvent('form_error', { form_id: 'trial_form', error_type: 'required_missing' })
      setDone('ng')
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        trackEvent('form_success', { form_id: 'trial_form' })
        setDone('ok')
        // 送信完了時はフォームの内容を保持したまま完了状態にする
      } else {
        trackEvent('form_error', { form_id: 'trial_form', error_type: 'request_failed' })
        setDone('ng')
        console.error('送信エラー')
      }
    } catch (err) {
      trackEvent('form_error', { form_id: 'trial_form', error_type: 'network_error' })
      console.error(err)
      setDone('ng')
    } finally {
      setSubmitting(false)
  ...
  return (
    <form data-track-form="trial_form" onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        {done !== 'ok' && (
          <button
            type="submit"
            data-track-cta="trial_form_submit"
            disabled={submitting}
